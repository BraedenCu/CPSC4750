...
def acoli_hist(initpos, foodfn, rttp, ttrp, ntrials, nsteps, nbins, 
    ...
    tumble state (a number between 0 and 1).

    TTRP is the probability of transitioning from a tumble ot a run (a number 
    between 0 and 1).
    ...
    if verbose: print('Trial:',end=' ',flush=True)
    for runi in range(ntrials):
        if verbose:
            print(runi,end=' ',flush=True)
